...
/* ================================================================
 * Command handlers — Virtual Memory Management
 * ================================================================ */

static int handle_mem_create(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemCreateRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_CREATE, -1, NULL, 0);
    ...
    prop.type = (CUmemAllocationType)req.type;
    prop.location.type = (CUmemLocationType)req.location_type;
    prop.location.id = req.location_id;

    CUmemGenericAllocationHandle handle;
    CUresult cr = cuMemCreate(&handle, (size_t)req.size, &prop, req.flags);

    GpuMemCreateResponse resp = { .handle = (uint64_t)handle };
    return send_response(ctx->fd, GPU_CMD_MEM_CREATE, (int32_t)cr,
                         &resp, sizeof(resp));
}

static int handle_mem_release(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemReleaseRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_RELEASE, -1, NULL, 0);

    GpuMemReleaseRequest req;
    memcpy(&req, payload, sizeof(req));

    CUresult cr = cuMemRelease((CUmemGenericAllocationHandle)req.handle);
    return send_response(ctx->fd, GPU_CMD_MEM_RELEASE, (int32_t)cr, NULL, 0);
}

static int handle_mem_address_reserve(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemAddressReserveRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_ADDRESS_RESERVE, -1, NULL, 0);
    ...
    CUdeviceptr ptr = 0;
    CUresult cr = cuMemAddressReserve(&ptr, (size_t)req.size,
                                       (size_t)req.alignment,
                                       (CUdeviceptr)req.addr, req.flags);

    GpuMemAddressReserveResponse resp = { .ptr = (uint64_t)ptr };
    return send_response(ctx->fd, GPU_CMD_MEM_ADDRESS_RESERVE, (int32_t)cr,
                         &resp, sizeof(resp));
}

static int handle_mem_address_free(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemAddressFreeRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_ADDRESS_FREE, -1, NULL, 0);

    GpuMemAddressFreeRequest req;
    memcpy(&req, payload, sizeof(req));

    CUresult cr = cuMemAddressFree((CUdeviceptr)req.ptr, (size_t)req.size);
    return send_response(ctx->fd, GPU_CMD_MEM_ADDRESS_FREE, (int32_t)cr, NULL, 0);
}

static int handle_mem_map(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemMapRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_MAP, -1, NULL, 0);
    ...
    CUresult cr = cuMemMap((CUdeviceptr)req.ptr, (size_t)req.size,
                            (size_t)req.offset,
                            (CUmemGenericAllocationHandle)req.handle,
                            req.flags);
    return send_response(ctx->fd, GPU_CMD_MEM_MAP, (int32_t)cr, NULL, 0);
}

static int handle_mem_unmap(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemUnmapRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_UNMAP, -1, NULL, 0);

    GpuMemUnmapRequest req;
    memcpy(&req, payload, sizeof(req));

    CUresult cr = cuMemUnmap((CUdeviceptr)req.ptr, (size_t)req.size);
    return send_response(ctx->fd, GPU_CMD_MEM_UNMAP, (int32_t)cr, NULL, 0);
}

static int handle_mem_set_access(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemSetAccessRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_SET_ACCESS, -1, NULL, 0);
    ...
    desc.flags = (CUmemAccess_flags)req.access_type;
    desc.location.type = (CUmemLocationType)req.location_type;
    desc.location.id = req.location_id;

    CUresult cr = cuMemSetAccess((CUdeviceptr)req.ptr, (size_t)req.size,
                                  &desc, (size_t)req.count);
    return send_response(ctx->fd, GPU_CMD_MEM_SET_ACCESS, (int32_t)cr, NULL, 0);
}

static int handle_mem_get_granularity(ConnectionCtx *ctx, const void *payload, uint32_t len)
{
    if (len < sizeof(GpuMemGetGranularityRequest))
        return send_response(ctx->fd, GPU_CMD_MEM_GET_GRANULARITY, -1, NULL, 0);
    ...
    CUresult cr = cuMemGetAllocationGranularity(
        &granularity, &prop, (CUmemAllocationGranularity_flags)req.option);

    GpuMemGetGranularityResponse resp = { .granularity = (uint64_t)granularity };
    return send_response(ctx->fd, GPU_CMD_MEM_GET_GRANULARITY, (int32_t)cr,
                         &resp, sizeof(resp));
}
...
static void *connection_handler(void *arg)
{
    ...
    while (g_running) {
        ...
        case GPU_CMD_MEM_CREATE:
            rc = handle_mem_create(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_RELEASE:
            rc = handle_mem_release(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_ADDRESS_RESERVE:
            rc = handle_mem_address_reserve(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_ADDRESS_FREE:
            rc = handle_mem_address_free(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_MAP:
            rc = handle_mem_map(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_UNMAP:
            rc = handle_mem_unmap(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_SET_ACCESS:
            rc = handle_mem_set_access(ctx, buf, hdr.payload_len);
            break;
        case GPU_CMD_MEM_GET_GRANULARITY:
            rc = handle_mem_get_granularity(ctx, buf, hdr.payload_len);
            break;
