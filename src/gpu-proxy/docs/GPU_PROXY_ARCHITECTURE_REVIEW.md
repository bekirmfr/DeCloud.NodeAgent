...
- **Identity-verified daemon adoption.** `GpuProxyService` no longer adopts a daemon on `pgrep` name-match alone. Adoption requires `/proc/<pid>/exe` to be the current `DaemonPath` (currency) and `/proc/<pid>/cmdline` to carry the intended `-i <mode>` (isolation mode); on mismatch the daemon is killed and a correct one launched (fail-safe — any uncertainty relaunches). Validated on WSL2: a hand-launched `threaded` daemon was rejected (`runs mode 'threaded', intended 'auto' — not adopting`), killed, and replaced with the agent's `-i auto` daemon; the currency path was independently confirmed (a non-daemon process matched by name was rejected as stale). Closes the "stale/wrong-mode daemon silently trusted" gap — the same permissive pattern as the original stale-daemon incident, one layer up.
