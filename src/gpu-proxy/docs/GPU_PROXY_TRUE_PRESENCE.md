...
**Updated 2026-06-25:** This is addressed by the **fork-per-VM worker model, shipped and hardware-validated**. The supervisor stays CUDA-free and forks a separate worker process per connection, each with its own primary CUDA context — cross-context access fails at the GPU MMU level, not at a validation layer. The **cross-tenant denial property is directly confirmed**: with two different-owner VMs, tenant B's read of tenant A's device address was DENIED under fork isolation and VULNERABLE under the threaded (shared-context) control. Worker/supervisor lifecycle and per-tenant VRAM quota are also confirmed on hardware.

**Remaining:** the vsock quota path is code-verified only (not testable on WSL2); the write/free/kernel-launch cross-tenant paths are closed by the same context boundary but were not each separately probed (read is the canonical test). See Architecture Review §5 and Debugging Journal Sessions 18–22.