...
public class LazysyncDaemon : BackgroundService
{
    ...
    {
        // Lazysync is replication and nothing else (Decision 18). With no local
        // BlockStore VM there is nowhere to push, so the cycle has no work —
        // skip before anything is frozen or snapshotted for a push that cannot
        // happen. This also makes SyncVmAsync's non-nullable blockstoreAddr
        // parameter honest again.
        var blockstoreAddr = await FindBlockstoreApiAsync(ct);
        if (blockstoreAddr == null)
        {
            _logger.LogDebug("LazysyncDaemon: no local BlockStore VM — skipping cycle");
            return;
        }

        var vms = _vmManager.GetRunningVms()
            .Where(v => // Replication-only enrolment: a VM with nothing to
                        // replicate has no reason to be in this cycle.
                        // Containers have no qcow2 overlay to snapshot — always
                        // true, now stated rather than assumed.
    ...
    private async Task SyncVmAsync(VmInstance vm, string blockstoreAddr, CancellationToken ct)
    {
        ...
        finally
        {
            ...
            // while disk.qcow2 stays frozen. Covers the "no changed chunks"
            // early return and every exception path. Kept from pass 1 — a real
            // pre-existing bug, unrelated to CSAM.
